...
function Call(props) {
  ...
  useEffect(() => {
    if (peerId) {
      ...
      socket.on("adminService", (p_admin_service_object) => {
        ...
        if (
          p_admin_service_object.connect &&
          p_admin_service_object.available
        ) {
          const id = p_admin_service_object.peerId;
          //request user media
          navigator.mediaDevices
            .getUserMedia({ video: { width: 120 }, audio: true })
            .then(function (stream) {
              //send caller details to admin.
              socket.emit("onCallWithAdmin", {
                name: "Admin",
  ...
  const controlUserVideo = () => {
    if (currentUserVideoRef.current && currentUserVideoRef.current.srcObject) {
      if (
        currentUserVideoRef.current.srcObject.getTracks()[0].kind === "video"
      ) {
  ...
  const controlUserMic = () => {
    if (currentUserVideoRef.current && currentUserVideoRef.current.srcObject) {
      if (
        currentUserVideoRef.current.srcObject.getTracks()[0].kind === "audio"
      ) {
  ...
  return (
    ...
    >
      ...
      <Draggable nodeRef={nodeRef}>
        <div ref={nodeRef} className="video-container" id="video-container">
          <div
            style={{
              display: "flex",
              flexDirection: "row",
